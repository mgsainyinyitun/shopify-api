...
public interface ProductRepository extends JpaRepository<ProductEntity,Long> {
    ...
    @Query( "SELECT p FROM ProductEntity p  WHERE p.id NOT IN (SELECT t.product.id FROM TradeHistoryEntity t WHERE t.user.id = :userId AND t.state='FINISHED') AND p.merchant.id = :merchantId ")
